...
final class CapabilityTests: XCTestCase {
    private func decode(_ json: String) throws -> CapabilityReport {
        try JSONDecoder().decode(CapabilityReport.self, from: Data(json.utf8))
    }

    /// The grade is the point of the page. If the app cannot tell a probe from
    /// a sentence someone wrote, it is a README with a tick next to it.
    func testEvidenceGradesSurviveTheWire() throws {
        let r = try decode("""
        {"source":"daemon","assessed":"running-vm","scope_dir":"/w","capabilities":[
          {"id":"hvf","title":"Hypervisor.framework","support":"yes","evidence":"observed","detail":"x"},
          {"id":"gicv2m","title":"GICv2M","support":"no","evidence":"documented","detail":"y"}
        ]}
        """)
        XCTAssertTrue(r.isFromDaemon)
        XCTAssertEqual(r.capabilities[0].evidence, .observed)
        XCTAssertTrue(r.capabilities[0].evidence.isMeasured)
        XCTAssertEqual(r.capabilities[1].evidence, .documented)
        XCTAssertFalse(r.capabilities[1].evidence.isMeasured)
        XCTAssertEqual(r.measuredCount, 1)
        XCTAssertEqual(r.documentedCount, 1)
    }

    /// A newer daemon may send a verdict this build has never seen. It must not
    /// be able to talk this app into rendering a green tick.
    func testAnUnknownVerdictDoesNotBecomeAYes() throws {
        let r = try decode("""
        {"capabilities":[{"id":"x","title":"X","support":"probably","evidence":"vibes","detail":"d"}]}
        """)
        XCTAssertEqual(r.capabilities[0].support, .unknown)
        // An ungradeable claim is the weakest grade, not the strongest: nothing
        // has shown it was measured.
        XCTAssertEqual(r.capabilities[0].evidence, .documented)
        XCTAssertEqual(r.measuredCount, 0)
    }

    /// Compiled-in must never read as measured. This is the exact conflation
    /// that made `is_available` return true for an unentitled binary.
    func testBuiltIsNotMeasured() {
        XCTAssertFalse(CapabilityEvidence.built.isMeasured)
        XCTAssertFalse(CapabilityEvidence.documented.isMeasured)
        XCTAssertFalse(CapabilityEvidence.recorded.isMeasured)
        XCTAssertTrue(CapabilityEvidence.probed.isMeasured)
        XCTAssertTrue(CapabilityEvidence.observed.isMeasured)
    }

    /// An absent preflight is a fact about the reader, not a clean bill of
    /// health for a capture nobody named.
    func testNoSnapshotInScopeIsDistinctFromNoObjections() throws {
        let r = try decode("""
        {"source":"daemon","assessed":"no-snapshot-in-scope","scope_dir":null,"capabilities":[]}
        """)
        XCTAssertTrue(r.hasNoSnapshotInScope)
        XCTAssertNil(r.preflight)
        XCTAssertNil(r.scopeDir)
    }

    func testPreflightCarriesRefusalsAndSummary() throws {
        let r = try decode("""
        {"source":"daemon","assessed":"running-vm","scope_dir":"/w","capabilities":[],
         "preflight":{"dir":"/w","readable":true,"refusals":1,"degraded":1,"unknowns":0,
          "summary":"1 of 8 checks refuse this snapshot",
          "findings":[{"id":"memory","title":"Guest RAM","support":"no","evidence":"probed","detail":"short"}]}}
        """)
        let p = try XCTUnwrap(r.preflight)
        XCTAssertEqual(p.refusals, 1)
        XCTAssertEqual(p.findings[0].support, .no)
        XCTAssertFalse(p.summary.lowercased().contains("supported"))
    }

    /// A report that did not come from the daemon describes a binary this app is
    /// not talking to.
    func testAReportWithoutProvenanceIsNotTrusted() throws {
        let r = try decode("""
        {"capabilities":[{"id":"hvf","title":"H","support":"yes","evidence":"probed","detail":"d"}]}
        """)
        XCTAssertFalse(r.isFromDaemon)
    }

    /// Claims must keep stable identities so SwiftUI does not collapse or
    /// reorder rows that say different things.
    func testClaimsHaveDistinctIdentities() throws {
        let r = try decode("""
        {"capabilities":[
          {"id":"a","title":"A","support":"yes","evidence":"probed","detail":"1"},
          {"id":"b","title":"B","support":"no","evidence":"documented","detail":"2"}
        ]}
        """)
