...
| A container-derived guest had **no network and no disk**, because the kernel used at the time built virtio as modules and a container rootfs ships no `/lib/modules` | [#222](https://github.com/gimbal-dev/gimbal-local/issues/222) | Partly fixed. #228 warns when a kernel cannot give the guest devices; #230 configures the NIC. **Note:** the original claim that *every* downloadable arm64 distro kernel is modular was later **disproven** — Ubuntu `generic` has virtio built in and is the verified path |
