...
| **V9.9 ★** (#155) | **Opt-in ingress** — reach a named port inside a sandbox | **G20** | **Two-thirds shipped, and the remaining third is the honest part.** The transport landed in #330 and V11 drove a real browser through it, so this issue's original premise — that a service inside a sandbox has no way to be reached — no longer holds: `--expose <port>` is opt-in, per-port, refuses the `host:guest` form and duplicates by name, and binds host loopback through a constant rather than a flag. What is left is that the sandbox can neither **describe** nor **report** the door: `sandbox.json` has no ingress vocabulary, and `chm posture` — the one command whose job is to say what is switched on — has no ingress row, so an exposed port is invisible to it. A mechanism that is silent about a control it has weakened is the failure this repo has banked seven times, one layer up. | S |
