...
| **G3** | **Runs Copilot for real, safely isolated** | ✅ **done — proven end-to-end on a rehydrated capture 2026-08-13 (#286)** | V7.1 proved it on a cold boot. #286 then proved the hard half: inside a **rehydrated Graviton2 capture** on the shipped v0.2.0 binary, the Copilot CLI wrote `fizz.py` and ran it holding **no credential** — injection verified against a control (200 with, 401 without). The guest was then **suspended and resumed twice**, and the agent **read back its own file and extended it**. Three agent runs, exit 0 each time, guest uptime continuous at 14.09 days. The native binary ran **without** `NODE_OPTIONS=--jitless`, so the earlier 5-of-5 SIGILL measurement is stale. Peak 105 live address spaces against the 256 the 8-bit ASID ceiling allows, 0 crashes — so #279 is not a prerequisite for this workload. The friction found on the way is #315–#318. |
...
| **G6** | **Create local images — from vanilla *or from containers*** | ✅ **done** | Vanilla: cold boot, BYO images. **Containers: `chm image build alpine:3.20`** pulls from a registry, verifies every blob by digest, and writes an image that cold-boots on HVF to a shell (V9.7, #153). The default rootfs is a **cpio initramfs**, which removes the `root=` question rather than answering it. For larger or writable roots, `--disk` now writes an ext2 image the guest mounts; measured on `node:22-slim` with writes surviving a fresh boot (#205 / PR #308). Open: zstd layers (#206), `--platform` (#207). |
...
| **G8** | **Snapshot management** | ✅ **done** | Lineage, fork, rollback, bounded growth, retention roots (pin/unpin), disk-usage reporting, **delete, gc and label** (see [`snapshot-retention.md`](snapshot-retention.md)) and **export/import** (see [`snapshot-export.md`](snapshot-export.md)). Deleting a parent is measured not to strand its descendants: a 2 vCPU guest resumed to a login shell from a revision whose parent had been deleted, and the bytes the command reports reclaiming (1.4 GiB) match what the volume gave back (1452 MiB). A written checkpoint now re-runs the resume-time drift guard against itself, so a torn capture fails at write time rather than becoming a revision nobody can resume. **V9.5c closed the last piece:** a 25 GiB lineage exports to a 2.7 GiB content-addressed bundle and imports back for 2.9 GiB, because the chunk grid is aligned to the delta writer's own and the overlays' holes are left as holes — and an imported revision was measured resuming to a live shell, which is the only claim that matters. **V9.14 closed the last caveat:** the test writer now shares `commit_checkpoint` with the product instead of mirroring it, and the expected on-disk shape comes from a listing taken off a real lineage rather than from the code under test — because sharing stops the two drifting but not moving together (#180). |

### Goals we already held, stated explicitly

| # | Goal | State | Evidence, or what is missing |
| --- | --- | --- | --- |
| **G9** | **Every claim carries its evidence** — the app never asserts a control it has not checked, **and never withholds a conclusion it has reached** | ✅ **done** | V6.5 capability honesty; `chm posture` exits non-zero when anything is weakened; the proxy page names which process answered. The first-run empty state reports the discovery rejection's own reason and remedy rather than a generic failure (V8.5). The second half was added by V9.11 (#192), after three separate "nothing works" reports turned out to be **the app reaching a correct conclusion and delivering it where nobody would look** — a refusal in the Activity log behind an enabled button, a settings notice on a page you had no reason to open. A control that is enforced silently is indistinguishable from a broken product. |
| **G10** | **A build someone else can actually run** | ✅ **done** | Signed with a Developer ID, **notarized by Apple and stapled**, so it opens on a Mac that has never seen this repo. Verified as a stranger receives it: unzipped outside any checkout, `spctl` → `accepted, source=Notarized Developer ID`, ticket validated offline, then it **cold-booted a real Alpine guest** and **resumed a vanilla Graviton2 KVM snapshot** (`6.8.0-136-generic/aarch64`). #144 / V8.6. |
...
| --- | --- | --- | --- |
| **G15 ★** | **A declarative sandbox spec** — one document that says what a sandbox *is*: image, sizing, egress policy, credential rules, env, entrypoint, lifetime | ✅ **done, with named gaps** | **V9.3 (#150).** `sandbox.json` — and it deliberately does **not** use our own vocabulary. [nebuk89/Dev-spec @ agent-compute-spec](https://github.com/nebuk89/Dev-spec/tree/agent-compute-spec) enumerates `hostRequirements.hypervisor` as `["firecracker", "cloud-hypervisor", "qemu", "any"]` — **we are one of four named targets**, so a document written for that spec should describe *this* sandbox. Hence `resourceLimits.memory.ram`, `networkPolicy`, `secrets`, camelCase, unit-suffixed strings. The honesty mechanism is that `--spec` expands to argv and re-enters **the same parser** as the flags, so a spec has no private route to the guest and the two forms are byte-identical (measured). **One named gap, a refusal rather than silence:** spec sections we have not built (`extensions`, `securityModules`, `dataPolicy`, `identity`, `observability`, lifecycle hooks — #182–#189). A second category existed at ship — fields we understood and could not *carry into a guest* — and **V9.10 (#190) emptied it**: `env` and `postBootCommand` now reach a real guest. The category itself is kept, because the next describable-but-undeliverable field needs somewhere to be declared rather than dropped. |
...
| **G18 ★** | **Idle sleep with activity reset, and a graceful stop** | ✅ | **V9.6 (#172).** A deadline or idle window now *suspends* to a resumable checkpoint rather than cutting power — G2's mechanism, reused, and affordable only because V9.1a made a capture a delta. The idle default moved 10s → 600s, matching Cloudflare's `sleepAfter`: 10s was scaffolding for a build that could not model virtio devices, never a judgement about idleness. Activity resets on console output; that is a proxy, and measuring vCPU WFI residency instead is #171. |
| **G19 ★** | **Named sizing tiers** | ✅ | **V9.3 (#150).** `small` 1×1GB · `standard` 2×2GB · `large` 4×4GB · `xlarge` 8×8GB, resolvable by name in a spec and overridable by an explicit field or flag. A tier is a chm convenience, not an agent-compute-spec concept — it resolves *into* `resourceLimits`, so a document that names one still says what it means. Hardware-verified: `tier: standard` booted a guest reporting `aarch64/2/1960`. |
| **G20 ★** | **Ingress — reach a service inside the sandbox** | 🟡 **transport done; the sandbox cannot yet describe or report it** | The NAT is no longer egress-only. `chm create --net --expose <port>` arms one host listener per named guest port (#330, `befc2faa8`), and V11 drove a real workload through it end to end: host Playwright → NAT ingress → in-guest forwarder → Chromium, with a page navigated, its DOM read back and a PNG rendered inside the guest. Concurrency measured at **96 parallel flows, ok=96 fail=0**, all payloads byte-identical. Three of #155's five requirements are met and were checked rather than assumed: it is **opt-in** (`--expose` needs `--net`), **per-port and explicit** (one bare port; the `host:guest` form and a repeated port are both refused by name), and **bound to host loopback** — `INGRESS_BIND_ADDR` is deliberately a constant rather than a flag, because a flag that accepts `0.0.0.0` is how a sandbox gets published to the LAN by accident. Missing, and both are description rather than transport: `sandbox.json` has no way to name an exposed port, so a spec cannot say what a spec-started sandbox publishes; and **`chm posture` does not mention ingress at all**, so a sandbox can be reachable from the host while the report that exists to say what is switched on stays silent. That second one is the sharper gap — it is a claim without its evidence, which is G9's whole subject. |
...
| **G22 ★** | **Env vars and entrypoint at start** | ✅ **done, with a stated scope** | **V9.10 (#190).** V9.3 gave both fields a home and then *refused* them, because nothing could carry either into a guest — caught pre-ship by asking *where does this value end up?* rather than *does it parse?*. The delivery uses the one channel a BYO image always has: the console, and the shell on it. **Readiness is proven, not pattern-matched** — a framed no-op comes back or it does not, so no prompt string has to be guessed at. `env` is exported by the console's own shell rather than handed to `sh -c`, because `export` in a subshell exits 0 and sets nothing — the exact false-sell shape this milestone exists to close. Measured in a guest: `V910_INSIDE=delta7734`, and the export still visible in the operator's own session. The scope is **stated rather than implied**: it reaches the post-boot command, later `exec`s and your session, and *not* a fresh login after that shell exits, which would need a writable rootfs no BYO image owes us. |
...
| **C2** | **The workspace implementation is independently built.** No prior-system crate, library, binary, service, schema, migration, wire protocol, manifest, storage format, test or repository content may be depended on, vendored, forked, linked, imported, executed, copied or adapted. No claim of read/write compatibility with a prior-system volume or checkpoint. | [`living-workspaces.md`](living-workspaces.md) §1. FUSE, copy-on-write layers, content-addressed blocks and three-way merge are **requirements to implement, not permission to reuse**. Phase 0 carries a dependency-and-provenance inspection as an exit gate. |
...
### The most recent milestones, and which goal each served

Reverse chronological, all merged and hardware-verified:

| # | Milestone | Merged | Serves |
| --- | --- | --- | --- |
| 1 | **V11.3 · `chm posture` carries the ingress row it always owed** — **I14 has been written down since V11.0**, with a full evidence column citing `NatResponder::expose` and `INGRESS_BIND_ADDR`, listed in the default-posture table, and marked *Holds today* — and `chm posture` never carried a row for it. So a sandbox could be reachable from the host on loopback while the report that exists to answer *“what can this sandbox do”* exited **0** and said nothing: a documented invariant with no executable check, which is the exact G9 failure the command was built to prevent. **The existing posture-coupling guard structurally could not have caught it** — it requires every `CHM_STRICT_*` env var to appear in a row, and ingress is armed by a CLI flag on `chm create`, so the gap looked closed precisely because the guard that polices posture coverage cannot see flags. `--expose` is now recorded in the live-run registry (`chm ps` gained an `EXPOSED` column) and the report reads it. **Three outcomes, not two, and the third is the point:** a registry that cannot be *read* is **weakened**, never *not applicable* — posture is a gate, and exiting 0 on a control that was never checked is the mechanism-reports-safety-it-does-not-provide shape this repo has banked seven times. It is also the one row that **escapes the “whose posture is it?” trap by construction**: every other control resolves from the environment of the process computing it, so `chm posture` and `chm ctl posture` can legitimately disagree, while a published port is a property of a live run and both forms read the same machine-wide registry. **Seven mutations fire.** The instructive one is M6: the call-site guard was **blind on its first draft**, because `&args.expose` appears at *two* sites in `create.rs` — the registry call and the code that actually opens the ports — so a whole-file needle stayed green against a `create.rs` that registered nothing. Repaired with a depth-balanced paren scan of the call itself, and **M6b is what proves the repair**: mutating the *other* site correctly leaves the guard green, so it fails for the right reason rather than merely being able to fail. Eighth instance of the call-site class here. Honest limit, stated rather than implied: nothing guards the plumbing call site, and regressing it **over**-reports a port as published, which is the fail-safe direction (#155) | 08-17 | **G9**, G20, G4 |
| 2 | **V11 · The browser sandbox — an agent gets a browser and nothing else** — inspired by kernel.sh, and the first workload built *on* this stack rather than *into* it. An agent reaches a Chromium it cannot otherwise touch: host Playwright `connectOverCDP` → NAT ingress → an in-guest byte forwarder → `headless_shell`, with a real page navigated, its DOM read back, `evaluate()` returning a computed value and a PNG rendered inside the VM. Five phases, each measured: **#330** gave the NAT its first inbound path (`--expose`, 96 parallel flows, ok=96 fail=0); **#331** built the arm64 image; **#339** wrote the 2,095-byte forwarder; **#332** committed the drive as a gate; **#333** proved the differentiator — a **warm browser is a resumable artifact**, and an *in-memory* session cookie issued before a checkpoint came back after **two** consecutive resumes, which cannot be a reload from the profile because `grep -rac` found the nonce in zero files on disk while the `Cookies` database sat right there. **Two of my own design calls were wrong and hardware corrected both:** `bind(0.0.0.0, 9222)` fails **EADDRINUSE** against Chromium's own loopback listener, so the shape written into #339 could not have worked at all rather than merely been weaker, and the `--remote-debugging-pipe` bridge recommended over a byte relay deletes the HTTP endpoint `connectOverCDP` requires — it would have had to *become* a DevTools front door inside the guest, where the relay parses nothing. The security guard is the **syscall-vocabulary decode against an allow-list**, not byte reproducibility: smuggling `execve` into the assembly fired both guards while `the_binary_is_reproducible_from_the_source` correctly stayed green, which is what proves which of the two carries the claim. **One acceptance mutation did not fire** — drifting `CDP_PORT` left all 13 checks green, because the forwarder patches its ports in at install time and the gate runs against a **prebuilt** image, so the mutation never reached the guest. That limit is now recorded in the script rather than tuned away: reading the constant stops the *script* drifting from the *tree*, and does not verify the *image* was built from the tree. Also measured and worth keeping: peak RSS **526 MiB, 17.5%** of the 3008 MiB ceiling, so memory was never the risk; `/dev/shm` peaked at 1.3 MiB against a 981 MiB default, so it needs no sizing; boot to CDP ready **2 s**; and Chromium keeps **its own sandbox** as uid 65432 under working user namespaces, so `--no-sandbox` is the fallback and not the default (#329, #330, #331, #332, #333, #339) | 08-17 | **G20**, G1, G9 |
| 3 | **V9.22 · The app asks the engine whether a kernel can boot** — the app classified kernels **by filename**, so Alpine's `vmlinuz-virt` — the file Alpine actually ships, and the first thing a new user has — was refused while the engine had long since learned to unwrap gzip and EFI zboot. A filename is not a format, and the control run proved it **cuts both ways**: on `main`, a directory holding a real bootable `vmlinuz-virt` was refused with *"No kernel"*, and 4 KiB of `/etc/services` renamed to `Image` was **accepted**. New `chm kernel probe <PATH> [--json]` makes the engine's own `kernelimage::decode` answerable from outside, and the app carries its verdict verbatim rather than restating its rules — the #192/#202/#203 shape, one layer down. The call-site mutation (production `scan` stops passing the real prober) **did not fire**, for the fourth time in this repo, so the parameter lost its default and omitting it is now a **compile error** rather than something a guard has to catch. Three doc surfaces still telling users the kernel must be uncompressed were corrected, since they had become false advice (#242) | 08-07 | **G1**, G9 |
| 4 | **V9.19 · A container guest gets the virtio drivers its kernel left out** — V9.16 made the problem legible; this closes it. `chm image build --modules <DIR>` resolves the virtio dependency closure for the kernel it was handed (5 modules, 238 KiB out of 916 in an Alpine `linux-virt` tree), installs it, and generates an init that loads it before the interface is configured. **Two silent failures the hardware found and no test could:** all five modules reached the archive and *none* reached the guest, because `init/initramfs.c` never creates a missing parent — `openat` fails ENOENT and the kernel drops the file without a word, the same class as the usr-merge symlink trap through a different door; and on a rootfs with no `insmod` the modules loaded and the NIC still never appeared, because virtio_mmio probes on a **workqueue**, so five `insmod` fork/execs were slow enough to win a race chm's single-process loader lost every time. A module's init returning is not the device being ready. `--modules` without `--kernel` is refused **before any bytes are pulled**, because the release to match comes from the kernel's own banner. The warning now withdraws only the drivers actually bundled, and `chm create` answers "will this guest have the driver" by **reading the initramfs**, not the metadata. Proved twice on hardware: alpine (musl, has `insmod`) reached `registry.npmjs.org`, and debian:12-slim (glibc, **no `insmod`**, the #224 case) came up with `eth0`, a default route, and a TCP connection to `deb.debian.org:443` via the 808-byte static loader. Six mutations fire — including the call-site one that did not, whose source guard then **matched its own assertion text** and had to have its needle assembled from parts (#222) | 08-07 | **G1**, G6, G9 |
| 5 | **V9.15 · A guest configures its own NIC** — a cold-booted container guest came up with `eth0` present, addressless and useless: chm attaches the device and runs the NAT, and nothing inside the guest ever brings the interface up. The generated init now does it, trying `ip` then `ifconfig` and, when a rootfs ships neither, printing the address, prefix, gateway and the two commands to run — because configuring an interface is an **ioctl** and no shell builtin makes one, so a refusal that names the exact values is the most honest thing a script can do. The addresses are **read** from `create.rs`, not restated, since a drifted literal would put the guest on a different subnet from its own gateway and every test would still pass. The netmask derivation found a real bug on its first run — a `/0` prefix shifted a `u32` by 32 and panicked. Three mutations fire. Verified three ways on hardware: `alpine:3.20` reached `registry.npmjs.org`; a build with the `ip` branch renamed away produced an **identical** result via `ifconfig`; and `node:22-slim`, which has neither tool, printed all five refusal lines with correct addresses **and still booted to a shell** (#227) | 08-07 | **G1**, G9 |
| 6 | **V9.16 · A kernel that cannot drive virtio says so at boot** — a container guest with no network and no disk looks like a hypervisor fault and is not: the kernel built virtio as modules and a container rootfs ships no `/lib/modules`, so the driver and its bus can never meet. chm now inspects the kernel it is handed and warns by name. **The original diagnosis was wrong and is retracted in the docs**: the claim that every downloadable arm64 distro kernel is modular came from testing one, and Ubuntu `generic` has virtio built in (#222, #228) | 08-07 | **G1**, G9 |
| 7 | **V9.17 · A container guest gets a controlling terminal** — `/bin/sh: can't access tty; job control turned off` was the first line every cold-boot user read, and it was not cosmetic: **Ctrl-C did not interrupt anything**. chm generates the init, so the fix is ours — `setsid -c`, with a re-entry guard so the handover cannot loop. The guard test initially passed a mutation that removed `-c`, because the generated script's own explanatory *comment* contains the string it was matching: it was asserting on prose, not code (#226, #229) | 08-07 | **G1**, G9 |
| 8 | **V9.18 · `--platform` and zstd layers** — a platform this host cannot boot is refused **before a byte is fetched**, rather than pulling real layers and writing a real image directory that can never start. Compression is detected by **magic bytes, never `mediaType`**, because `read_blob` never sees a media type and registries mislabel. Proved against a real alpine rootfs recompressed as zstd and served from a local TLS registry: 517 entries, identical to the gzip build (#206, #207) | 08-06 | **G6** |
| 9 | **V8.6 · A build someone else can actually run** — signed with a Developer ID, **notarized by Apple and stapled**, so the `.app` opens on a Mac that has never seen this repo, offline. The release script runs the test gate **in the configuration it ships**, and that is not ceremony: the first release build hung on every boot because `fcntl` was declared with a fixed third argument, and on Apple arm64 variadic arguments go on the stack while fixed ones go in registers — so `O_NONBLOCK` was never set, `drain_pipe` blocked forever, and every vCPU parked before one guest instruction. Debug passed a benign zero and worked. **Correct tests existed and passed the whole time, because the suite had only ever run in debug.** Reverting the declaration is now a compile error. Verified as a stranger receives it — unzipped outside any checkout, `spctl accepted / source=Notarized Developer ID`, then it cold-booted Alpine to a shell and **resumed a vanilla Graviton2 KVM snapshot** (#144) | 08-06 | **G10**, G9 |
| 10 | **V9.14 · A checkpoint format change is visible to the tests** — every checkpoint test built its input with a hand-written *mirror* of the real writer, so the pair agreed by construction and a layout change moved one side while the suite stayed green. That is exactly how #178 made **every checkpoint on the machine** refuse to resume. The mirror had already drifted: no `.tmp` staging, no atomic rename, no bracketed fingerprint, no self-verification — so rollback, prune, export and import were all tested against an artifact the product never produces. `commit_checkpoint` is now shared by both writers. **Sharing stops them drifting; it does not stop them moving together** — measured, by adding one sidecar file to `write_checkpoint` and watching all 514 tests pass. So the expected shape comes from *outside* the code: a directory listing taken off a real Graviton lineage written by shipped builds. Four mutations fire, and re-mirroring the test writer is caught by the source guard while the shape fixture stays green — which is why both exist. Proved on hardware by resuming a shipped-build checkpoint on the refactored writer and then resuming *its* output to `RAMPROOF=charlie4471`, a RAM-only variable (#180) | 08-06 | **G9**, G8 |
| 11 | **V9.7 · Containers become bootable images** — `chm image build alpine:3.20` pulls from Docker Hub, verifies every blob by digest, and writes an image that cold-boots on HVF to an interactive shell. The rootfs ships as a **cpio initramfs**, not a filesystem image: macOS has no `mkfs.ext4` and `hdiutil` only makes HFS/APFS, and the initramfs shape *removes* the `root=` question rather than answering it, killing the whole PARTUUID-mismatch failure class. Two guards were found **worthless by mutation**: the hardlink test measured host-side bytes, which stay small whether `nlink` is 1 or 411 — but `init/initramfs.c::maybe_link()` opens with `if (nlink >= 2)`, so below that the guest lands 410 zero-byte binaries that exist and do nothing. Measured: `python:3.12-alpine` runs Python 3.12.13 and `pip` at the 512 MiB its own `image.json` declares (#153) | 08-06 | **G6**, G1 |
| 12 | **V9.11a · The app says why it refused** — V9.11 gave **Start** a named, visible refusal and left the **Open terminal** button with the same silent `appendLog` + `return` it had always had, which is the button the report was about. It is *not* the slot check called in one more place: Start and Open terminal fail for different sets of reasons, so chm being spawnable, Terminal.app being present and the image still being in the library are enumerated too, and **every** blocker is reported rather than the first — clearing one and finding another waiting is how a fixable setup comes to feel like a broken app. A missing workspace is deliberately **not** a blocker, because the app recreates one from the image and that self-healing is what lets a workspace be deleted to reclaim space. A sandbox bound to an abandoned library is **stated and not acted on** — silently moving someone's disk is the same class of unannounced action #192 existed to stop. Six mutations proved to fire, including the call site (#195, #196) | 08-06 | **G9**, G1 |
| 13 | **V9.5c · Snapshot export / import** — a lineage moves between machines as a content-addressed bundle, and the sharing survives **both** directions: 50.0 GiB apparent exports to **2.7 GiB** and imports back for **2.9 GiB**. The chunk size is the delta writer's own 64 KiB grid, so an unchanged region hashes identically in every revision; import clones the previous revision and patches only the differing chunks; all-zero chunks are left as holes. **Two false sells caught on hardware, neither by a test:** the first import wrote every revision out in full (24 GiB), and the fixed one produced revisions that verified, deduplicated and *could not be resumed*, because writing the files moved the mtimes #139 fingerprints. Modification times are now carried, and the import checks its output against the fingerprint the **source machine** shipped — the one check here with power a writer/reader pair cannot have. Proved by resuming an imported revision to a live 2-vCPU shell (#177) | 08-06 | **G8**, G14, G2 |
| 14 | **V9.10 · `env` and `postBootCommand` reach a real guest** — V9.3 refused both because nothing could deliver them; this builds the channel. Readiness is *proven* (a framed no-op comes back) rather than matched against a prompt string a BYO image never promised. **The hardware run found a second bug the tests could not:** `--post-boot` is greedy by design, so a spec expanding to it swallowed the operator's own flags and `chm create --spec … --dry-run` **booted a VM when asked to describe one**. Specs now expand to a non-greedy `--post-boot-arg` (#190) | 08-06 | **G22**, G15, G9 |
| 15 | **V9.11 · The app says what it knows** — three "nothing starts" reports, one disease: settings validated *existence* rather than *usability* (a present-but-empty library passed silently), the single-HVF-slot refusal went to the Activity log while Start stayed enabled, and the daemon's report of a running guest was gated on in-memory state so a live sandbox read as `Stopped` after an app restart. Seven mutations proved to fire (#192) | 08-06 | **G9**, G1 |
| 16 | **V9.3 · The declarative sandbox spec** — `sandbox.json` says what a sandbox *is*, in the [agent-compute spec](https://github.com/nebuk89/Dev-spec/tree/agent-compute-spec)'s own vocabulary rather than ours. `--spec` expands to argv and re-enters the same parser as the flags, so the two forms are byte-identical and a spec gets no private route to the guest. Sections we have not built are **refused by name with an issue number**, never silently ignored (#150) | 08-06 | **G15**, G19, G7 |
| 17 | **V9.6a · A checkpoint proves it can describe its own overlays** — `write_checkpoint` brackets the overlay copy and then re-runs *the resume-time guard* against what it just wrote, so a torn capture fails at write time instead of stranding a revision nobody can resume (#179). Plus frozen on-disk fixtures, because the format regression in #178 was green in every test by construction (#180) | 08-06 | **G9**, G2 |
| 18 | **V9.5b · Snapshot reclaim: delete, gc, label** — delete a revision and be told what it really gave back; collect what no reader can reach; name a point so a timeline of timestamps becomes a list of reasons. Fixes a resume regression that had made **every existing checkpoint** refuse to start (#178) | 08-06 | **G8**, G2 |
| 19 | **V8.5 · A first-run empty state that teaches** — names what an image *is*, surfaces the discovery rejections up front, and removes a gate that greyed out cold boot whenever the snapshot library was empty (#175) | 08-05 | **G9**, G1 |
| 20 | **V9.6 · A deadline suspends instead of cutting power** — `--max-seconds`/`--idle-exit` save a resumable checkpoint, and a latent #139 bug where a checkpoint's own teardown invalidated its own fingerprint is fixed (#172) | 08-05 | **G18**, G2 |
| 21 | **V9.1a · Delta RAM dumps + reclaim-honest accounting** — a live snapshot rewrites only the 64 KiB chunks that changed, and usage reports what deleting a revision would actually give back (#167) | 08-05 | **G2**, G8 |
| 22 | **V9.1 · Continuous snapshots** — checkpoint a *running* guest on a cadence; a session that ends badly keeps its work (#148) | 08-05 | **G2**, G8 |
| 23 | **V9.5a · Retention roots + honest disk accounting** — pin a revision so age-based pruning cannot reclaim it, and report what a lineage really costs (#152, part) | 08-04 | **G8**, G2 |
...
**The one true blocker is gone.** V8.6 shipped 2026-08-06: there is a signed,
...
What that run also found was a coherent product gap rather than a hypervisor
defect: the obvious path a new user takes — build a container image, start it
from the app, run an agent in it — had potholes at every step (#222, #224,
#225, #226). V9.18 is now closed.

| | Milestone | Goal | Why now | Size |
| --- | --- | --- | --- | --- |
| **V9.18 ★** (#222, #224, #225, #226) | **The first-run path actually works** | ✅ **done** | Measured from a clean machine on the released binary. The path now names and fixes the four potholes the acceptance run found: modular virtio drivers are bundled by `chm image build --modules` (#222); the app shows guests started by either the app or CLI through the run registry (#225); Ctrl-C works because the guest gets a controlling terminal (#226); and image builds name the rootfs libc so agent workloads can avoid the musl Copilot CLI failure (#224). None of these was a hypervisor defect; together they decide whether the product feels finished. | M |
| **V9.8 ★** (#156) | **Runtime-mutable egress policy** | **G17** | Change what a sandbox may reach without throwing away its work. | M |
| **V9.9 ★** (#155) | **Opt-in ingress** — reach a named port inside a sandbox | ✅ **done** | **Shipped in three passes, and the last one found the defect.** The transport landed in #330 and V11 drove a real browser through it; #346 gave `chm posture` its I14 row; this pass named ingress in `sandbox.json`. The vocabulary is **adopted from upstream unchanged** — `networkPolicy.ingress[]` with `allow`/`deny`/`host`/`port`/`protocol` — and then **narrowed, refusing every widening dimension by name**, because the spec models a *filter* over traffic that would arrive anyway and `--expose` creates a *listener*. `deny` is the sharpest refusal: accepting it would be a no-op wearing the shape of a control, which is the #179 shape. **The defect this milestone found is in the half that already shipped:** `resolve()` called `port_number().ok()`, so a rule whose *port* is valid but whose `host` is refused published anyway — and the doc comment claimed the safety lived there. Production was never exposed, because `expand_spec` validates first, but a comment asserting a property the code does not have is exactly what this project treats as a defect. Cured with one predicate: `refusals()` collects every reason, `validate` reports from it and `publishable_port()` gates on it. **Proven structurally, not by the guard**: with `expand_spec`'s `validate()` deleted, the real binary rendered **zero** `--expose` flags for a document naming a refused port. Ten guards, eight mutations, and one of them — a content assertion blind to a re-hardcoded `127.0.0.1` — did not fire, which is why the loopback claim now carries a paired source guard. | S |
...
| **V9.15 ★** (#183–#189) | **The spec sections we refuse** — `extensions` (the "needs bash/node" requirements shape), `securityModules`/`securityProfile`, `dataPolicy`, `identity`, `observability`, lifecycle hooks | G15 | Each is refused **by name with its issue number**, so a spec written against the full agent-compute schema tells its author exactly which parts this build will not honour. That is the permanent contract for a local tool that will never implement every section — and the #178/#180 lesson applied: silently dropping `securityModules` would start a sandbox weaker than its own description. | M each |
| **V9.16 ★** (#182) | **Decide the conformance target** | G15 | Full conformance with the agent-compute schema, or the narrower and possibly more honest *"reads a conforming document and refuses what it cannot honour"*? The second may be the right permanent answer for a single-user local tool, but it should be a decision rather than a default. | S |
| **V9.17 ★** (#170) | **Better UX for egress on snapshots** | G4, G9 | A resumed snapshot inherits an egress posture its author may not have chosen, and nothing in the UI makes that visible. Explicitly *not* a request to flip the resume default — that was considered and rejected. | S |
...
| --- | --- | --- | --- | --- |
| **V10.0 ★** (#159) | **Contract and spike** — a throwaway guest FUSE frontend talking to a minimal workspace service over the private network route | G24 | A real build runs from the mount; **and a dependency/provenance inspection proves no prior-system material is present** (C2). | L |
...
| **4. A coding agent works inside it.** | ✅ **true on a rehydrated snapshot, measured 2026-08-13 (#286)** | The **GitHub Copilot CLI wrote and ran code inside a rehydrated Graviton2 capture holding no credential**, which was attached at the network edge — verified against a control rather than by trusting the agent: the same request returned **200 with injection and 401 without**, so the credential demonstrably arrived and the guest demonstrably never held it. This row previously read “true on a cold-booted guest”, and the distinction mattered exactly as stated: sentence 1 of the dream is about a cloud snapshot, so an agent proven only on a guest with no snapshot in the path did not finish sentence 4. It does now — and the resume half is the part that makes it a product rather than a slower cold boot: after **two suspend/resume cycles** the agent read back the file it had written before the first suspend and extended it. §5 has the transcript. |
...
| **V8** | **A local app that ships on its own** | ①③ | ✅ **complete 2026-08-06** — cold boot and BYO images from the app, a local-only mode that actually stops calling a control plane, a first run that teaches, and **a signed, notarized build someone else can run** (V8.6/#144). §5a |
...
7. ~~**V8.4–V8.6 — finish the local MVP.**~~ — ✅ done 2026-08-06. Settings
   persist, the empty state teaches, and there is a signed, notarized build
   that opens on a Mac that has never seen this repo.
...
### The line: what lives on the Mac

The Mac-side tree contains the **app** (`app/GimbalLocal`), the **engine +
daemon** (`chm`, `hypervisor/src/hvf/`), and the thin **runner client** that
calls the plane when one exists. The control plane contains the server on the
...
arm64 `Image` kernel, optionally an initramfs and raw disks, optionally an
...
| --- | --- |
| a file that is not a kernel in any form | whatever `chm kernel probe` says — an x86 bzImage is named as such, so is a file that is not a kernel at all |
...
names what an image *is* — a folder holding an arm64 `Image` kernel — says
...
| | Milestone | Why | Size |
| --- | --- | --- | --- |
| ~~**V8.6** (#144)~~ | ✅ **A build someone else can run** — shipped 2026-08-06 | Signed with a Developer ID, notarized, stapled. The `.app` bundles an entitled `chm` and resolves `chmPath` from the bundle. **[§0a](#0a-how-milestones-ladder-into-the-goals) ranked this the one true blocker on the word "ship"; it is gone.** | M |
