...
| **G6** | **Create local images — from vanilla *or from containers*** | ✅ **done** | Vanilla: cold boot, BYO images. **Containers: `chm image build alpine:3.20`** pulls from a registry, verifies every blob by digest, and writes an image that cold-boots on HVF to a shell (V9.7, #153). The rootfs is a **cpio initramfs**, not a filesystem image — macOS has no `mkfs.ext4`, and the initramfs shape *removes* the `root=` question rather than answering it. Measured on real Docker Hub images: `busybox` restores its 411-way hardlink group, `python:3.12-alpine` runs Python 3.12.13 and `pip` inside the 512 MiB its own `image.json` declares. Open: a disk-backed variant for images too large to unpack into RAM (#205), zstd layers (#206), `--platform` (#207). |
