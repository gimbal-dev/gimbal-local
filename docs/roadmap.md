...
| **G3** | **Runs Copilot for real, safely isolated** | 🟡 **proven on cold boot; not yet end-to-end on a rehydrated snapshot** | V7.1: Copilot CLI installed, authenticated, wrote and ran JS, holding **no credential** — verified by hashing, not by trusting the agent. **That was a cold-booted guest, and the distinction is not a technicality**: a cold guest reads this Mac's own `CTR_EL0` and is immune by construction to the DIC delta that breaks JITs in a *rehydrated* one. On a rehydrated Graviton capture the CLI does install (`npm i -g @github/copilot` exits 0, measured 2026-08-08) and `npm` needs `NODE_OPTIONS=--jitless` to run at all — 10 of 10 failures without it. The repeated-resume wedge that blocked earlier attempts is closed (#257 / PR #309). The remaining rehydrated-agent acceptance gap is #286. |
...
| **4. A coding agent works inside it.** | 🟡 **true on a cold-booted guest, hardware-proven 2026-08-03; not yet on a rehydrated snapshot** | The **GitHub Copilot CLI installed, authenticated, and wrote and ran a JavaScript app** inside a Gimbal Local guest — and did it **holding no credential**, which was attached at the network edge. Verified independently of the agent's own claim, and the real token's sha256 appears nowhere in the guest. §5 has the transcript. **The route was the cold-booted guest V6.8 predicted would be immune**, and reading this row as covering the rehydrated case is the mistake it used to invite: sentence 1 of the dream is about a cloud snapshot, so an agent proven only on a guest with no snapshot in the path does not finish sentence 4. Measured 2026-08-08 on a rehydrated Graviton capture: the CLI installs cleanly, and `npm` needs `NODE_OPTIONS=--jitless` (10 of 10 failures without it). The repeated-resume wedge is closed (#257 / PR #309); the remaining clean-machine rehydrated-agent acceptance is #286. |
