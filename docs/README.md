...
| [`container-images.md`](container-images.md) | **Turning an OCI/Docker image into a bootable sandbox** (`chm image build`) — and, up front, the three limits that bite in the first ten minutes: you must bring a kernel and most arm64 distro kernels build virtio as *modules* so the guest gets no NIC or disk; some kernels are EFI zboot rather than raw `Image`; and the rootfs unpacks into guest RAM. Also why agent workloads want a glibc base. |
