...
| `CHM_SNAPSHOT_INTERVAL_SECS=<n>` | Checkpoint the *running* guest every `n` seconds, without stopping it. Off by default, and **outranked by `--snapshot-every`** on `run`/`resume`/`connect` — including `--snapshot-every 0`, so a caller inside an environment it does not own can still opt out. Use the variable to turn the cadence on for everything a wrapper launches; use the flag for one run. | Long agent sessions, where a session that ends badly should not be a session whose work is gone. Each checkpoint freezes the guest while it captures RAM — **measured 0.9–2.1 s for a 2 GiB guest on an M-series Mac**, costing 2–13 MiB of disk, since V9.1a writes each snapshot as a delta against the last. `chm` reports the freeze it actually measured every time, so the interval is a trade you make on your own numbers. See `docs/continuous-snapshots.md`. |
| `CHM_USGIC_LEAK_ACTIVE=<n>` | **Deliberately break the guest.** Drop the `n`th deactivation of the virtual-timer PPI on each vCPU, leaving that INTID active forever — the exact shape of the #302 defect, which is what [#257](https://github.com/gimbal-dev/gimbal-local/issues/257) looked like. The vCPU's tick never returns; its siblings are unaffected. | Proving the wedge detector can actually fire. A detector that has never reported anything is indistinguishable from a guest that has never wedged, and this is the difference. It earned its place on its first run: it showed the detector's crossing test was a one-in-5.8-million race that could not have fired on hardware. Inert unless set; `n` must be reached, so a value above a run's real deactivation count silently does nothing. |
