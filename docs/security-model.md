...
| **Deliverable-interrupt routing** (I7) | **on, structural** | — |
| **CAS digest validation** (I8) | **on, structural** | — |
| **Egress allow-list** (I9) | **off** — see below | `chm firewall set` to turn *on* |
| **Guest-port ingress** (I14) | **off** — nothing inside a guest is reachable from the host | `chm create --expose <port>`, one named port at a time, on loopback |
| **Browser-guest CDP forwarder** (I15) | **on, structural, browser images only** — one port, one address, no configuration surface | — |
| **Same-authority widening** (I13) | **on, structural** — a rules file widens egress only for its own `host:port` pairs, and only when it shares the policy's authority | — |
...
**One row escapes the trap by construction: guest ingress (I14).** `--expose` is
a flag on `chm create`, so it is neither an environment binding nor a workspace
file, and it has already been consumed by the time a guest is running. The only
thing that records it is the live-run registry, which is machine-wide — so both
forms read the same source and both answer for the whole Mac. That is the
correct scope rather than a compromise: a published port binds host loopback, so
*anything* on this machine can dial it, not just the workspace that opened it.
...
| I7 | **Undeliverable snapshots are never mis-run.** A capture whose virtio completions route through the GIC ITS as LPIs is rehydrated onto the userspace GICv3, which can deliver them; it is never run on the managed GIC, which cannot. | Structural: the managed-GIC runtime path is retired, so the userspace GICv3 is the only backend `chm run` and `chm serve` can reach. `CHM_ALLOW_ITS_LPI` no longer selects anything. | **Holds today, structurally** (V6.6 — was "routed by a decision", now "the only path") |
...
| I14 | **A port inside a guest is reachable from the host only where somebody named it, and only from this Mac.** No inbound path exists unless `--expose <port>` armed one; each exposure is a single guest port on its own OS-chosen **loopback** host port, so nothing is reachable from the LAN and two sandboxes cannot collide. An unparseable or ambiguous exposure spec refuses rather than defaulting. Inbound flows are counted against `NatLimits` like any other. | `NatResponder::expose` binds `INGRESS_BIND_ADDR` (`127.0.0.1`, a constant with no flag that widens it) and is the only thing that creates an inbound listener; `chm create --expose` accepts a bare port and nothing else. Mutation-tested guards in `nat::relay_test` cover the byte path, the unexposed port, the bind address and 96 concurrent flows. Every armed exposure is recorded in the live-run registry and reported by `chm posture` as a weakened control, so an open door cannot pass a gate silently. | **Holds today** (V11.0; reported since V11.3) |
| I13 | **An egress policy is only ever widened by rules of the same authority, and only where it was already going to be intercepted.** Credential-proxy rules imply an egress allowance for their own `host:port` pairs, so naming a host once is enough — but only when the rules and the policy came from the same authority (flag, environment, or workspace file), so a local file cannot reopen a host a governed policy closed. Implied entries are appended to `allow` (deny still wins), never lift the reserved-address guard, and carry their provenance into the decision they produce. A policy that could not be resolved is never widened. | Authority tagging in `credproxy::cli::resolve_rules` and `imp::resolve_egress_policy`; `RuleSet::implied_egress_allow` emits `host:port` only; `EgressPolicy::allow_implied` appends with a `note` rendered by `Rule::describe`. | **Holds today** (V8.7) |
| I15 | **The browser guest's CDP forwarder relays one port and can do nothing else.** The browser image ships a freestanding aarch64 relay because Chromium's DevTools endpoint binds guest loopback and has no switch to move it, so I14's ingress would otherwise dial an address with nothing on it. The relay binds the guest's own NIC address on the one CDP port — never a wildcard, so nothing already inside the guest gains a path it did not have — dials guest loopback on that same port, and forwards bytes without parsing them. It never creates a process, never opens a file, and reads no configuration: the address and port are patched in from `create::GUEST_IP` and `browser::CDP_PORT` before the image is written, and `argv` is ignored entirely. It is one thread over a fixed 128-slot `epoll` table, so it allocates nothing and back-pressures on the kernel's backlog rather than dropping. | `chm/src/oci/cdpfwd.S`, installed by `oci::browser::install`. `cdpfwd::tests::the_syscall_vocabulary_is_exactly_the_allow_list` decodes every `svc` out of the shipped bytes against a 15-call allow list that contains no `execve`, `clone` or `openat`. Mutation-tested guards in `oci::cdpfwd` drive the real binary in a `linux/arm64` container and cover the single listening socket, `argv` that cannot widen it, 96 and 200 concurrent flows, and half-close. | **Holds today** (V11.2a) |
